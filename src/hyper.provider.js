		...
		function resourceFactory(path){
			...
			Resource.prototype.Get = function(params){
				...
				function getLink(steps, link, deferred){
					...
					/* Check Cache First */
					var cached = $window.sessionStorage[link];
							...
							function(result){
								if(step){
									...
									if(nextLink){
										...
										return deferred.reject('unable to find link for `' + step + '` in available payloads');
									}	
								}else{
									// we found our data
				...
				// kick this thing off
				return getLink(this.Steps.slice(0), ApiRoot);
			};
